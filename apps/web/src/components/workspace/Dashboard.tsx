...
interface DashboardProps {
  ...
  onCreateHtmlArtifact: (title?: string, folderId?: string | null) => Promise<PageItem | null>;
...
export function Dashboard({
  ...
  async function handleCreateHtmlArtifact() {
    const page = await onCreateHtmlArtifact();
    if (page) navigate(`/page/${page.id}`);
  }
  ...
  const tiles = (
    <div className="grid" style={{ gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: 12 }}>
      ...
      />
      <QuickTile
        color="var(--color-gold)" title="HTML artifact" subtitle="Code + live preview" onClick={handleCreateHtmlArtifact}
